...
export default function EditInvoiceScreen() {
  ...
  useEffect(() => {
    const checkForIncomingItems = async () => {
      ...
      try {
        ...
        setProcessingItems(false); // Reset flag on error
      }
    };
  ...
  }, [invoice, invoiceId, processingItems]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      handleIncomingItems();
    }, [invoice, invoiceId, processingItems]) // Add processingItems dependency
  );
  ...
  const refreshInvoice = async () => {
    try {
      ...
      if (tempSaleItems && Array.isArray(tempSaleItems) && tempSaleItems.length > 0 && invoice) {
        ...
        // Update stored data
        const currentInvoices = await Storage.getObject<SaleInvoice[]>(STORAGE_KEYS.SALES_INVOICES);
        if (currentInvoices) {
  ...
  const handleSaveInvoice = async () => {
    if (!invoice || !formData.customerName || !formData.phoneNumber) {
  ...
  const addItem = () => {
    // Set mode for sales items and mark that we're editing an invoice
    Storage.setObject('ITEM_SELECTION_MODE', 'sales');
    Storage.setObject('EDITING_INVOICE_ID', invoiceId);
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.customerInputContainer}>
            ...
            />
            {showCustomerDropdown && formData.customerName.length > 0 && (() => {
